 ...
 * @category  Controller
 */
class VacancyController extends SecureController {

    function __construct() {
        parent::__construct();
        $this->tablename = SqlTables::tbl_vacancy;
        $this->fields = [
            "id",
            "title",
            "description",
    ...
    function index($fieldname = null, $fieldvalue = null) {
        ...
        if (!empty($request->search)) {
            ...
            $this->view->search_template = "vacancy/search.php";
        }
        ...
        if (!empty($request->orderby)) {
            ...
            $db->orderBy("tbl_vacancy.id", ORDER_TYPE);
        }
        ...
        $page_title = $this->view->page_title = "Vacancy";
        $this->view->report_filename = date('Y-m-d') . '-' . $page_title;
        $this->view->report_title = $page_title;
        $this->view->report_layout = "report_layout.php";
        $this->view->report_paper_size = "A4";
        $this->view->report_orientation = "portrait";
        $this->render_view("vacancy/list.php", $data); //render the full page
    }
    ...
    function view($rec_id = null, $value = null) {
        ...
        if ($value) {
            ...
            $db->where("tbl_vacancy.id", $rec_id);
        }

        $record = $db->getOne($tablename, $this->fields);

        if ($record) {
            $page_title = $this->view->page_title = "View Vacancy";
            $this->view->report_filename = date('Y-m-d') . '-' . $page_title;
            $this->view->report_title = $page_title;
            $this->view->report_layout = "report_layout.php";
            $this->view->report_paper_size = "A4";
            ...
            return $this->render_view("vacancy/view.php", $record);
        } else {
            if ($db->getLastError()) {
                $this->set_page_error();
            } else {
...
//                    $modeldata['type'] = $this->type;
                ...
                if ($rec_id) {
                    ...
                    $upload_data = $uploader->upload($_FILES['file'], $upload_config);
                    ...
                    if ($upload_data['isSuccess']) {
                        ...
                        foreach ($filepaths as $file) {
                            ...
                            if (!$db->update($this->tablename, [
                                        'file' => $file['name'],
                                        'file_original_name' => $file['old_name']
                                    ])) {
                    ...
                    if (!empty($_FILES['other_files'])) {
                        ...
                        if ($upload_data['isSuccess']) {
                            ...
                            foreach ($filepaths as $file) {
                                ...
                                if (!$db->insert(SqlTables::tbl_vacancy, $data)) {
                                    $errors[] = $db->getLastError();
                                }
                            }
                        } else {
            ...
            ajaxFormPostOutcome($errors, get_link(Router::$page_name));
            return;  
        }
        ...
        if (!empty($rec_id)) {
            ...
            if ($record) {
                $this->view->page_title = 'Edit Vacancy: ' . $record['title'];
                $this->view->page_props = $record;
                return $this->render_view("vacancy/manage.php");
            } else {
                if ($db->getLastError()) {
                    $this->set_page_error();
                } else {
            ...
            $this->view->page_title = "New Vacancy";
            return $this->render_view("vacancy/manage.php");
        }
    }
    ...
    function delete($rec_id = null) {
        ...
        $db->where("tbl_vacancy.id", $arr_rec_id, "in");
