...
class ServiceController extends SecureController {
    ...
    function __construct() {
        parent::__construct();
        $this->tablename = SqlTables::tbl_service;
        $this->fields = [
            "id",
            "name",
            "description",
    ...
    function index($fieldname = null, $fieldvalue = null) {
        ...
        if (!empty($request->search)) {
            ...
            $search_condition = "(
				tbl_service.id LIKE ? OR 
				tbl_service.name LIKE ? OR 
				tbl_service.description LIKE ? 
			)";
            $search_params = array(
                "%$text%", "%$text%", "%$text%"
            );
            ...
            $this->view->search_template = "service/search.php";
        }
        ...
        if (!empty($request->orderby)) {
            ...
            $db->orderBy("tbl_service.id", ORDER_TYPE);
        }
        ...
        $page_title = $this->view->page_title = get_lang('service_title');
        $this->view->report_filename = date('Y-m-d') . '-' . $page_title;
        $this->view->report_title = $page_title;
        $this->view->report_layout = "report_layout.php";
        $this->view->report_paper_size = "A4";
        $this->view->report_orientation = "portrait";
        $this->render_view("service/list.php", $data); //render the full page
    }
    ...
    function view($rec_id = null, $value = null) {
        ...
        if ($value) {
            ...
            $db->where("tbl_service.id", $rec_id);
        }
        ...
        if ($record) {
            ...
            return $this->render_view("service/view.php", $record);
        } else {
            if ($db->getLastError()) {
                $this->set_page_error();
            } else {
    ...
    function manage($rec_id = null) {
        ...
        if (is_post_request()) {
                    ...
                    if (!empty($_FILES['other_files'])) {
                        ...
                        if ($upload_data['isSuccess']) {
                            ...
                            foreach ($filepaths as $file) {
                                ...
                                if (!$db->insert(SqlTables::tbl_service_file, $data)) {
                                    $errors[] = $db->getLastError();
                                }
                            }
                        } else {
            ...
            ajaxFormPostOutcome($errors, get_link("service"));
            return;
        }
        ...
        if (!empty($rec_id)) {
            ...
            if ($record) {
                $record['files'] = $files;
                $this->view->page_title = get_lang('edit_service') . ': ' . $record['name'];
                $this->view->page_props = $record;
                return $this->render_view("service/manage.php");
            } else {
                if ($db->getLastError()) {
                    $this->set_page_error();
                } else {
            ...
            $this->view->page_title = get_lang('new_service');
            return $this->render_view("service/manage.php");
        }
    }
    ...
    function delete($rec_id = null) {
        ...
        $db->where("tbl_service.id", $arr_rec_id, "in");
    ...
    function delete_service_file($rec_id = null) {
        Csrf::cross_check();
        $request = $this->request;
        $db = $this->GetModel();
        $this->rec_id = $rec_id;
        ...
        foreach ($arr_rec_id as $id) {
            $db->where("id", $id);
            $file = $db->get(SqlTables::tbl_service_file);

            if ($file) {
                unlink(UPLOAD_FILE_DIR . $file['path']);
                $db->where("id", $id);
                $db->delete(SqlTables::tbl_service_file);
            }
        }

        $this->set_flash_msg(get_lang('record_deleted_successfully'), "success");
        return $this->redirect((isset($request->redirect) ? $request->redirect : 'service'));
    }

    function delete_service_cover_image($rec_id = null) {
        Csrf::cross_check();
        $request = $this->request;
        $db = $this->GetModel();
        $db->where("id", $rec_id);
        $record = $db->get(SqlTables::tbl_service);

        if ($record) {
            unlink(UPLOAD_FILE_DIR . $record['cover_image']);
            $db->where("id", $rec_id);
            $db->update(SqlTables::tbl_service, [
                'cover_image' => NULL,
