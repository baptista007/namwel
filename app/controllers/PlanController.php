 ...
 * @category  Controller
 */
class PlanController extends SecureController {

    function __construct() {
        parent::__construct();
        $this->tablename = SqlTables::tbl_plan;
        $this->fields = [
            "id",
            "name",
            "description",
    ...
    function index($fieldname = null, $fieldvalue = null) {
        ...
        if (!empty($request->search)) {
            ...
            $search_condition = "(
				tbl_plan.id LIKE ? OR 
				tbl_plan.name LIKE ? OR 
				tbl_plan.monthly LIKE ? OR 
				tbl_plan.trimestral LIKE ? 
			)";
            $search_params = array(
                "%$text%", "%$text%", "%$text%", "%$text%"
            );
            ...
            $this->view->search_template = "plan/search.php";
        }
        ...
        if (!empty($request->orderby)) {
            ...
            $db->orderBy("tbl_plan.id", ORDER_TYPE);
        }
        ...
        $page_title = $this->view->page_title = get_lang('plan_title');
        $this->view->report_filename = date('Y-m-d') . '-' . $page_title;
        $this->view->report_title = $page_title;
        $this->view->report_layout = "report_layout.php";
        $this->view->report_paper_size = "A4";
        $this->view->report_orientation = "portrait";
        $this->render_view("plan/list.php", $data); //render the full page
    }
    ...
    function view($rec_id = null, $value = null) {
        ...
        if ($record) {
            ...
            return $this->render_view("plan/view.php", $record);
        } else {
            if ($db->getLastError()) {
                $this->set_page_error();
            } else {
    ...
    function manage($rec_id = null) {
        ...
        if (is_post_request()) {
            ...
            $this->sanitize_array = array(
                ...
                'monthly' => 'sanitize_string',
                'trimestral' => 'sanitize_string',
                'speed' => 'sanitize_string',
            ...
            if (empty($errors)) {
                ...
                if ($rec_id) {
                    if (!empty($_FILES['files'])) {
                        ...
                        $upload_data = $uploader->upload($_FILES['files'], $upload_config);
                        ...
                        if ($upload_data['isSuccess']) {
                            ...
                            foreach ($filepaths as $file) {
                                ...
                                if (!$db->insert(SqlTables::tbl_plan_file, $data)) {
            ...
            ajaxFormPostOutcome($errors, get_link("plan"));
            return;
        }
        ...
        if (!empty($rec_id)) {
            ...
            $db->where("plan_id", $rec_id);
            $files = $db->get(SqlTables::tbl_plan_file);

            if ($record) {
                $record['files'] = $files;
                $this->view->page_title = get_lang('edit_plan') . ': ' . $record['name'];
                $this->view->page_props = $record;
                return $this->render_view("plan/manage.php");
            } else {
                if ($db->getLastError()) {
                    $this->set_page_error();
                } else {
            ...
            $this->view->page_title = get_lang('new_plan');
            return $this->render_view("plan/manage.php");
        }
    }
    ...
    function delete($rec_id = null) {
        ...
        $db->where("tbl_plan.id", $arr_rec_id, "in");
    ...
    function delete_plan_file($rec_id = null) {
        Csrf::cross_check();
        $request = $this->request;
        $db = $this->GetModel();
        $tablename = $this->tablename;
        ...
        foreach ($arr_rec_id as $id) {
            $db->where("id", $id);
            $file = $db->get(SqlTables::tbl_plan_file);
            
            if ($file) {
                unlink(UPLOAD_FILE_DIR . $file['path']);
                $db->where("id", $id);
                $db->delete(SqlTables::tbl_plan_file);
            }
        }
        
        $this->set_flash_msg(get_lang('record_deleted_successfully'), "success");
        return $this->redirect((isset($request->redirect) ? $request->redirect : 'plan'));
    }
}
