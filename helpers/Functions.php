...
function public_user_login_status() {
    return (!empty(get_session('pub_user_data')) ? true : false);
}
...
function ajaxFormPostOutcome($errors, $redirectUrl = null, $success_message = null, $runThis = null) {
    if (!empty($errors)) {
        if (is_array($errors)) {
            $msg = "<ul>";
...
function strip_tags_from_db($string) {
    return strip_tags(htmlspecialchars_decode($string));
}

function format_currency($price, $currency = null) {
    return ($currency ? $currency : CURRENCY) . ' ' . number_format($price);
}

function html_to_no_html($string) {
